...
def progress_gap(progress, time_progress):
    displayed_progress = pct(progress)
    displayed_time_progress = pct(time_progress)
    if displayed_progress is None or displayed_time_progress is None:
        return None
    return displayed_time_progress - displayed_progress
...
def aggregate_channel(df):
    ...
    grouped["进度GAP"] = grouped.apply(
        lambda row: progress_gap(row["招生进度"], row["进度"]),
        axis=1,
    )
    ...
    for row in rows:
        ...
        for col in columns:
            ...
            if col in ["招生目标", "成单量", "量级GAP"]:
                ...
                bar_value = abs(value) if not pd.isna(value) else value
                cells.append(f'<td>{bar(bar_value, "#5b8ff9", signed_pct_text(value))}</td>')
            elif col == "状态":
...
def total_row(df, span_cols):
    ...
    gap = current - target
    progress_gap_value = progress_gap(progress, time_progress)
...
def lec1_channel_count(data, last_from, payment_values):
...
def render_lec1_share(demo, target, summary):
    ...
    headers = "".join(f"<th>{html.escape(name)}</th>" for name in ["渠道"] + names + ["合计"])
    rows = [
        "<tr><th>报量</th>"
        + cells(target_volumes, int_text, "num")
        + f'<td class="num">{int_text(target_total)}</td></tr>',
        "<tr><th>进量</th>"
        + cells(counts, int_text, "num")
        + f'<td class="num">{int_text(intake_total)}</td></tr>',
        "<tr><th>目标占比</th>"
  ...
  <main class="report lec1-report">
    <header>
      ...
      <div class="metrics">
        ...
        <div><span>进量合计</span><b>{int_text(intake_total)}</b></div>
