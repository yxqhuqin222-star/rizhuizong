...
const calculationRules = {
  ...
  rows: [
    ...
    ["字段归并", "未命中 target 的渠道", "常规外呼未命中 target 时保留为仅现状项；其他渠道按同一学部、期次、价体、年级寻找 target 渠道，唯一候选直接归属，多个候选优先常规外呼、其次 LEC内测，两者都没有时归为未报量，无候选保留为仅现状项"],
    ["目标", "目标", "同一统计维度下 target 表的目标之和"],
    ["目标", "target_time", "同一统计维度下 target 表的最晚 target_time"],
    ["目标", "进量日期", "同一统计维度下 target 表的最晚进量日期"],
    ["现状", "现状", "按学部、期次统计 demo 全量，不按 target 进量日期过滤；同一统计维度下按 custom_uid 去重计数，custom_uid 缺失的行分别计数"],
    ["现状", "下单日期", "同一统计维度下 demo 表的最近一次下单日期，用于明细展示"],
    ["现状", "当日现状", "按学部、期次、渠道、价体、年级和下单日期统计当日 demo 去重量；前端选中下单日期时用于替换明细和当前筛选汇总的现状口径"],
    ["对比指标", "差距", "现状 - 目标"],
    ["对比指标", "完成率", "现状 ÷ 目标；目标为 0 时留空"],
    ["对比指标", "进度", "当前日期统一取同学部、同一期次 demo 的最近下单日期；当前日期早于进量日期时为 0；进量日期至目标日前按业务进度日数 ÷ 总天数计算，周一休息不计入业务进度日，目标日前最高 5/6；目标日及以后为 100%；总天数为 6 天，结果限制在 0%～100%"],
