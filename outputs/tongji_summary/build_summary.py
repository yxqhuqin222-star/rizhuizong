...
def calculate_progress(row):
    ...
    elapsed_days = business_progress_days(row["进量日期"], row["进度日期"])
...
def write_outputs(summary, current_summary, target_summary, demo, target, out_dir=OUT_DIR):
    ...
    metadata = pd.DataFrame(
        [
            ...
            ["明细排序", "同一学部、期次和渠道下，优先按价体排列，再按学部业务年级顺序排列"],
            ["播报图期次口径", "以 target 表中的期次为准，小学、初中、高中、自拼各自仅播报目标表里的最新一期次数据；期次按春 < 暑 < 秋 < 寒、同季节按数字排序"],
            ["价体展示", "原始价体除以 100，去除无意义的小数位，例如 100→1、990→9.9、1880→18.8"],
            ["播报图渠道展示", "线索渠道二级分类 + 格式化后的价体"],
            ["播报图招生目标", "目标"],
            ["播报图剩余天数", "目标日及以后为0；目标日前为max(6-当前进度阶段,1)；当前进度阶段=进度×6"],
