...
- P0 验收：上传 demo 或 target 后，Summary、工作簿、六类播报图、页面指标和健康接口都能用当前数据验证。
...
| 每日播报闭环 | 常见做法需要截图、修图、上传、复制消息 | 看板内下载总进度、小学、初中、高中、自拼、lec内测小学量级图片，并可发送到钉钉机器人 | reload 成功后先生成 Summary、工作簿、HTML 和 PNG，再暴露下载与播报接口 |
...
- 写入：六类播报 PNG、`reports/daily_progress/manifest.json`、钉钉 markdown image message。
...
2. 本产品如何实现：四学部概览里的落后数量是按钮。点击后自动应用学部筛选和落后 chip，让用户直接进入需要跟进的行。
3. 交互流程：
...
重算完成
    |
    v
生成 总进度 / 小学 / 初中 / 高中 / 自拼 / lec内测小学量级 PNG
    |
    +-- 下载 --> 新标签打开图片
    |
...
| PNG 播报图 | 群播报和图片转发 | 固定宽度视觉输出 | 总进度、小学、初中、高中、自拼、lec内测小学量级 |
...
- 状态：与看板状态分类一致。
- 年级排序：所有年级视图、聚合表和播报图按业务顺序展示：一年级、二年级、三年级、四年级、五年级、六年级、初一、初二、初三、高一、高二、高三。
...
最小可用版本必须完成固定 Excel 读取、字段校验、Summary 统一计算、最新期次默认看板、筛选明细、CSV 与 XLSX 导出、六类播报图生成、下载入口、钉钉配置校验、健康检查和能证明核心口径的自动化测试。

### P1

完善线上只读同步、失败补偿队列、当前视图导出包含筛选汇总、日数据渠道/年级聚合、自然语言查询澄清流程、别名表解析、查询明细分页和导出。
...
| 播报图文件存在率 | 6/6 | reload 后检查六个 PNG 路径存在且文件大小大于 10KB | 少于 6 个或任一小于等于 10KB |
