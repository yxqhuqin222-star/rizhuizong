...
def calculate_progress(row):
    if pd.isna(row["target_time"]) or pd.isna(row["下单日期"]):
        return pd.NA

    progress = (TOTAL_DAYS - (row["target_time"] - row["下单日期"]).days) / TOTAL_DAYS
    return 1 if progress < 0 else progress
...
def aggregate_target(df):
    ...
    return (
        data.groupby(DIMENSIONS, dropna=False, as_index=False)
        .agg(目标=("目标", "sum"), target_time=("target_time", "max"))
    )
...
def build_summary(demo, target):
    ...
    summary = summary[
        DIMENSIONS + ["下单日期", "target_time", "目标", "现状", "差距", "完成率", "进度"]
    ]
...
def write_outputs(summary, current_summary, target_summary, demo, target, out_dir=OUT_DIR):
    ...
    metadata = pd.DataFrame(
        [
            ...
            ["总天数", TOTAL_DAYS],
            ["下单日期口径", "每个统计项下取最近一次日期"],
            ["进度计算", "进度=(总天数-(target_time-下单日期))/总天数"],
            ["播报图期次口径", "以 target 表中的期次为准，小学、初中、高中各自仅播报目标表里的最新一期次数据"],
            ["播报图渠道展示", "线索渠道二级分类 + 价体"],
            ["播报图招生目标", "目标"],
            ["播报图剩余天数", "总天数-(target_time-下单日期)"],
