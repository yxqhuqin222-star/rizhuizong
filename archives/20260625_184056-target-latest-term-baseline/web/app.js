...
function rowStatus(row) {
  if (Number(row["目标"]) > 0 && Number(row["现状"]) === 0) return { text: "未开单", cls: "" };
  if (row["进度"] !== null && Number(row["完成率"] || 0) < Number(row["进度"])) return { text: "落后", cls: "late" };
  if (Number(row["目标"]) > 0 && Number(row["完成率"] || 0) >= 1) return { text: "已完成", cls: "" };
  if (Number(row["目标"]) === 0 && Number(row["现状"]) > 0) return { text: "仅现状", cls: "" };
  return { text: "正常", cls: "" };
}
...
function bindEvents() {
  ...
  ["filterDepartment", "filterTerm", "filterChannel", "filterPayment", "filterKeyword"].forEach(id => {
  ...
  document.getElementById("exportLatest").addEventListener("click", () => location.href = "/download/summary?scope=latest");
  document.getElementById("exportAll").addEventListener("click", () => location.href = "/download/summary?scope=all");
  document.getElementById("exportCurrent").addEventListener("click", exportCurrentRows);
  document.getElementById("downloadWorkbook").addEventListener("click", () => location.href = "/download/workbook");
  document.getElementById("exportQuery").addEventListener("click", () => location.href = "/download/query");
  document.getElementById("naturalQueryButton").addEventListener("click", runNaturalQuery);
  document.getElementById("reportPrimary").addEventListener("click", () => location.href = "/download/report?dept=primary");
  document.getElementById("reportMiddle").addEventListener("click", () => location.href = "/download/report?dept=middle");
  document.getElementById("reportHigh").addEventListener("click", () => location.href = "/download/report?dept=high");
}
...
function exportCurrentRows() {
  ...
  const headers = tableColumns.filter(key => key !== "下单日期");
